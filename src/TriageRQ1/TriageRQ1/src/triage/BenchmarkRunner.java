...
public class BenchmarkRunner {
    ...
    private static final Random RANDOM = new Random(42); // seed cố định -> tái lập được

    /**
     * Chạy benchmark cho một cấu trúc với kích thước dữ liệu cho trước.
     *
     * @param structure cấu trúc cần đo (sẽ bị xóa sạch trước khi đo)
     * @param dataSize  số bệnh nhân trong cấu trúc khi đo
     */
    public BenchmarkResult run(TriageStructure structure, int dataSize) {

        // ---- 1. Warm-up (không ghi kết quả) ----
        ...
        for (int i = 0; i < ITERATIONS; i++) {
            // Tiền điều kiện: cấu trúc có dataSize phần tử
            ...
            // Xóa để giữ kích thước ổn định cho lần sau
    ...
    /** Sinh bệnh nhân ngẫu nhiên với priority 1-10 */
    ...
    /**
     * Mô phỏng tải 200 updates/phút (≈ 3-4 ops/giây) trong 60 giây.
     * Đo xem cấu trúc có duy trì < 50ms không dưới điều kiện thực tế.
     *
     * @return danh sách thời gian (ms) của từng thao tác
     */
    public List<Double> simulateLoad(TriageStructure structure, int dataSize) {
        ...
        // 200 updates/phút = 200 operations, xen kẽ để giữ size ổn định
        for (int i = 0; i < 200; i++) {
            if (i % 3 == 0) {
                // Priority update: extract + đổi priority + re-insert (size giữ nguyên)
                long start = System.nanoTime();
                ...
                // Extract to balance insert above (size -1)
                long start = System.nanoTime();
                structure.extractMin();
                times.add((System.nanoTime() - start) / 1_000_000.0);
            }
